// Canvas particles
const canvas = document.querySelector("#canvas1");
const ctx = canvas.getContext("2d");

(function () {
  const checkBox = document.querySelector("input");
  canvas.width = window.innerWidth;
  canvas.height = window.innerHeight;
  const particleArray = [];
  let color = 0;

  window.addEventListener("resize", () => {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
  });

  const mouse = {
    x: undefined,
    y: undefined,
    radius: 5,
  };

  /* function drawCircle() {
  ctx.fillStyle = "white";
  ctx.beginPath();
  ctx.arc(mouse.x, mouse.y, mouse.radius, 0, Math.PI * 2);
  ctx.fill();
} */

  function move(eventPro) {
    mouse.x = eventPro.x;
    mouse.y = eventPro.y;
    /*  if (eventPro.shiftKey) {
    animate();
  } else {
    drawCircle();
  } */
  }

  class Particle {
    constructor() {
      if (checkBox.checked) {
        this.x = mouse.x;
        this.y = mouse.y;
      } else {
        this.x = Math.random() * canvas.width;
        this.y = Math.random() * canvas.height;
      }

      this.size = Math.random() * 15 + 1;
      this.speedX = Math.random() * 3 - 1.3;
      this.speedY = Math.random() * 3 - 1.3;
      this.color = `hsl(${color}, 100%, 50%)`;
    }
    update() {
      this.x += this.speedX;
      this.y += this.speedY;
      if (this.size > 0.2) this.size -= 0.1;
    }
    draw() {
      ctx.fillStyle = this.color;
      ctx.beginPath();
      ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2);
      ctx.fill();
    }
  }

  function particleMake() {
    for (let i = 0; i < 2; i++) {
      particleArray.push(new Particle());
    }
  }

  /* (() => {
  let index = -1;
  while (++index < 100) {
    particleArray.push(new Particle());
  }
})(); */

  function handleParticle() {
    let index = -1;
    while (++index < particleArray.length) {
      particleArray[index].update();
      particleArray[index].draw();
      for (let j = index; j < particleArray.length; j++) {
        const dx = particleArray[index].x - particleArray[j].x;
        const dy = particleArray[index].y - particleArray[j].y;
        const distance = Math.sqrt(dx * dx + dy * dy);
        if (distance < 100) {
          ctx.beginPath();
          ctx.strokeStyle = particleArray[j].color;
          ctx.lineWidth = 0.3;
          ctx.moveTo(particleArray[index].x, particleArray[index].y);
          ctx.lineTo(particleArray[j].x, particleArray[j].y);
          ctx.stroke();
          ctx.closePath();
        }
      }
      if (particleArray[index].size <= 0.3) {
        particleArray.splice(index, 1);
        index--;
      }
    }
  }

  canvas.addEventListener("click", function (event) {
    move(event);
    particleMake();
  });

  canvas.addEventListener("mousemove", function (event) {
    move(event);
    particleMake();
  });

  /* canvas.addEventListener("wheel", function (event) {
  if (event.deltaY === 100) {
    mouse.radius--;
  } else {
    mouse.radius++;
  }
  console.log(event);
}); */

  function animate() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    //drawCircle();
    //ctx.fillStyle = "rgba(0, 0, 0, 0.1)";
    //ctx.fillRect(0, 0, canvas.width, canvas.height);
    handleParticle();
    color++;
    requestAnimationFrame(animate);
  }

  animate();

  setInterval(() => {
    particleMake();
  }, 100);
})();

// Canvas Clock
